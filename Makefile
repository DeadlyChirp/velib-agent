...
.DEFAULT_GOAL := help
.PHONY: help up down logs test test-front test-doc test-fiabilite test-persistance test-injections test-charge test-norace test-race-docker test-live test-integration cover fmt vet lint vuln check reset
...
test-fiabilite: ## Mesure la fiabilite du modele configure (docker compose up requis)
	python audit/fiabilite_modele.py --tours 10
